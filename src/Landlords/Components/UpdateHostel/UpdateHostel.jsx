...
const UpdateHostel = () => {
  ...
  // State for current images (from backend)
    ...
    const fetchHostel = async () => {
      try {
        ...
        setFormData({
          ...
          curtain_rods: !!data.curtain_rods,
          electricity_backup: !!data.electricity_backup,
          water_backup: !!data.water_backup,
          elevator: !!data.elevator,
          flooding_prone_area: !!data.flooding_prone_area,
          garbage_collection_included: !!data.garbage_collection_included,
  ...
  const handleSubmit = async (e) => {
    ...
    Object.entries(formData).forEach(([key, value]) => {
      form.append(`hostel[${key}]`, value);
    });
    // Append new images if any
    if (newImages.length > 0) {
      newImages.forEach((img) => {
        form.append("hostel[images][]", img);
      });
    }
    try {
      await axios.put(
        ...
        <div className="landlord-container">
          ...
          <div className="dashboard-body">
            <div className="dashboard-main">
              ...
              {hostel && (
                <form className="update-hostel-form" onSubmit={handleSubmit} encType="multipart/form-data">
                  {/* Show current images */}
                      ...
                      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                        {currentImages.map((img, idx) => (
                          <img
                            ...
                            style={{ width: '100px', height: '70px', objectFit: 'cover', borderRadius: '4px', border: '1px solid #ccc' }}
                          />
                        ))}
                      </div>
                    </div>
